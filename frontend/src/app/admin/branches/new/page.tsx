...
function NewBranchForm() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/restaurants/${restaurantId}/branches`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ name, address, restaurantId })
      });

      if (res.ok) {
        // Success! Go back to selection or dashboard
        router.push("/select-restaurant");
      } else {
        const errorData = await res.text();
        setError(errorData || "Error al crear la sucursal.");
      }
    } catch (err) {
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-foreground/60">Agrega una nueva ubicación a tu restaurante</p>
          </div>
        </div>

        <form onSubmit={handleCreate} className="space-y-6 glass dark:glass-dark p-8 rounded-3xl border border-border/50">
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-widest text-foreground/40 px-1">Nombre de la Sucursal</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-widest text-foreground/40 px-1">Dirección / Ubicación</label>
            <textarea
              required
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              className="w-full min-h-[100px] bg-background/50 border border-border/50 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-semibold resize-none"
              placeholder="Ej: Av. Central #123, frente al parque..."
            />
          </div>
          ...
          <Button type="submit" disabled={loading} className="w-full h-16 rounded-2xl text-lg font-bold shadow-xl shadow-primary/20">
            {loading ? <Loader2 className="animate-spin" /> : "Crear Sucursal"}
          </Button>
        </form>
      </div>
    </div>
