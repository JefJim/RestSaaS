...
## Next Implementation Steps (Exact Order)
1. [ ] **Restaurants table**: Ensure schema aligns with multi-tenant junction logic.
2. [ ] **Branches table**: Create new entity and map to Restaurants.
3. [ ] **UserRestaurants table**: Verify junction and role management.
4. [ ] **Endpoint → Get user restaurants**: Implementation for selection flow.
5. [ ] **Restaurant selector page**: UI for choosing the active business.
6. [ ] **Branch selector page**: UI for choosing the active sucursal.
7. [ ] **Save restaurantId + branchId in JWT**: Backend update to token generation.
8. [ ] **Filter all queries by restaurantId**: Update EF Core Global Query filters.
9. [ ] **Show current restaurant in dashboard navbar**: UI update for context awareness.

After these steps, the architecture will be fully scalable and correctly isolated.
